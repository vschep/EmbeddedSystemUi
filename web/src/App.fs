...
type Model = { Text: string; CountModel: Count.Model }
...
let init () : Model * Cmd<Msg> =
    let countModel, countCmd = Count.initLoad ()
    { Text = "FableStarter"; CountModel = countModel }, Cmd.batch [ Cmd.map CountMsg countCmd ]
...
let update (msg: Msg) (model: Model) : Model * Cmd<Msg> =
    ...
    | CountMsg countMsg ->
        match countMsg with
        | _ ->
            let newCountModel, countCmd = Count.update countMsg model.CountModel
